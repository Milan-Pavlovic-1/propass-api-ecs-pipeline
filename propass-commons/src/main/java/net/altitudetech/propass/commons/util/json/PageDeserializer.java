...
/**
 * Spring does not come with a ready solution for deserializing {@link Page}. This class does. Taken
 * from: https://stackoverflow.com/a/54730375
 */
public class PageDeserializer extends JsonDeserializer<Page<?>> implements ContextualDeserializer {
  ...
  public Page<?> deserialize(JsonParser p, DeserializationContext ctxt) throws IOException {
    final CollectionType valuesListType =
        ctxt.getTypeFactory().constructCollectionType(List.class, valueType);
    ...
    return list.isEmpty() ? Page.empty() : new PageImpl<>(list, PageRequest.of(pageNumber, pageSize), total);
  }

  /**
   * This is the main point here. The PageDeserializer is created for each specific deserialization
